...
const getPreparationSeconds = () => {
    const preparationSeconds = Number(process.env.REACT_APP_PROGRAMMING_ASSESSMENT_PREPARATION_SECONDS);

    return Number.isFinite(preparationSeconds) && preparationSeconds > 0
        ? Math.floor(preparationSeconds)
        : 0;
};

const ProgrammingAssessmentPreparation = () => {
    const [secondsLeft, setSecondsLeft] = useState(getPreparationSeconds);
